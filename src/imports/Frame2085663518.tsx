...
function Frame20({ isActive, onClick }: { isActive: boolean; onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`content-stretch cursor-pointer flex items-center justify-center px-[10px] py-[4px] relative shrink-0 transition-all rounded-[3px] ${
        isActive ? 'bg-gradient-to-r from-[#cfffe8] to-[rgba(44,246,152,0.75)]' : ''
      }`}
    >
...
function Frame21({ isActive, onClick }: { isActive: boolean; onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`content-stretch cursor-pointer flex items-center justify-center px-[10px] py-[4px] relative shrink-0 transition-all rounded-[3px] w-[77px] ${
        isActive ? 'bg-gradient-to-r from-[#cfffe8] to-[rgba(44,246,152,0.75)]' : ''
      }`}
    >
...
function Frame22({ activeTab, onTabChange }: { activeTab: 'service' | 'product'; onTabChange: (tab: 'service' | 'product') => void }) {
  return (
    <div className="bg-[#f5f5f7] content-stretch flex items-center p-[4px] relative rounded-[4px] shrink-0">
      ...
      <Frame20 isActive={activeTab === 'service'} onClick={() => onTabChange('service')} />
      <Frame21 isActive={activeTab === 'product'} onClick={() => onTabChange('product')} />
    </div>
  );
}
...
// Service Form Fields (Service Required + Project Budget side by side)
function ServiceFormFields() {
  return (
    <div className="content-stretch flex gap-[40px] items-start relative shrink-0 w-full">
      <ServiceRequiredField />
      <ProjectBudgetField />
    </div>
  );
}

function Frame17({ formType }: { formType: 'service' | 'product' }) {
  return (
    <div className="content-stretch flex flex-col gap-[40px] items-start relative shrink-0 w-full">
      <Frame10 />
      <Frame13 />
      {formType === 'product' ? <Frame16 /> : <ServiceFormFields />}
      <Frame24 />
    </div>
  );
}
...
function Frame1() {
  return (
    <div className="bg-black relative rounded-[100px] shrink-0 size-[36px] transition-all duration-300 group-hover:scale-90 group-hover:ring-2 group-hover:ring-[#111]">
...
function Frame() {
  return (
    <div className="group bg-[#01e067] content-stretch flex gap-[10px] h-full items-center overflow-clip pl-[24px] pr-[8px] py-[6px] relative rounded-[80px] shrink-0 hover:bg-[#00cc5c] transition-all cursor-pointer">
      <div className="-translate-x-1/2 absolute flex h-[130.588px] items-center justify-center left-[calc(50%-105.63px)] mix-blend-plus-lighter top-[-27.29px] w-[126.735px] group-hover:animate-[flare_0.6s_ease-in-out]" style={{ "--transform-inner-width": "1200", "--transform-inner-height": "21" } as React.CSSProperties}>
...
export default function Frame18() {
  ...
  return (
    <div className="content-stretch flex gap-[60px] items-center relative size-full">
      <Frame3 />
